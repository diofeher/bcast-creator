...
(function (global, document) {
    ...
    function setCuePosition(div, time, duration) {
        duration = duration;  // placeholder for now
        var left = (100 * time) / duration;
        div.setAttribute("style","width:0.5em; left:"+left+"%");
    }

    function resizeTimeLine() {
        var markers = document.getElementById('markers'), value;
        if (typeof InstallTrigger !== 'undefined')   // Firefox 1.0+
            value = window.innerWidth - 90;          // length of markers in timeline
        else {
            if (window.innerWidth < 800)
                value = window.innerWidth - 225;
            else
                value = 575;
        }
        markers.style.width = value +'px';
    }
    ...
    function playBrowserCast() {
        ...
        slideCues.forEach(function (cue) {
            ...
            if(cue instanceof SlideCue) {
                setCuePosition(div, cue.time, popcorn.duration());
            }
